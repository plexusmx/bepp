...
public class ResultadosMedicoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        {
            nombre = bundEpecialidad.getString("nombreDoctor");
        ...
        USER_CLIENT_RETROFIT_PACIENT.getDoctorsInfo(nombre).enqueue(new Callback<List<Doctor>>() {
            @Override
            public void onResponse(Call<List<Doctor>> call, Response<List<Doctor>> response) {
                ...
                if(response.code() == 200){
                    ...
                    mAdapter = new EspecialidadAdapter( ResultadosMedicoActivity.this, doctors);
                    mListView.setAdapter(mAdapter);
                    mAdapter.setMode(Attributes.Mode.Single);

                } else if(response.code() == 500){
                    Toast.makeText(ResultadosMedicoActivity.this, "Error en el servidor", Toast.LENGTH_LONG).show();

                }else if(response.code() == 403){


                    Toast.makeText(ResultadosMedicoActivity.this, "La sesión ha expirado", Toast.LENGTH_LONG).show();
